...
class TestRailsAutolink < Minitest::Test
  ...
  def test_auto_link_email_addres_with_especial_chars
    ...
    email_raw_encoded = ERB::Util.url_encode(email_raw).gsub('%40', '@')
  ...
  def test_auto_link
    ...
    email2_raw_encode = ERB::Util.url_encode(email2_raw).gsub('%40', '@')
    email2_result = %{<a href="mailto:#{email2_raw_encode}">#{email2_raw}</a>}
